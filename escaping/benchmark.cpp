...
int main(int argc, char **argv) {
  ...
  if(!amd64_simple_needs_escaping(justquotes.c_str(), 64)|| !amd64_simple_needs_escaping(justbackslash.c_str(), 64)) {
    printf("AMD64 simple implementation is not correct\n");
    return 1;
  }
  pretty_print(data.size(), volume, "AMD64 simple_needs_escaping",
            bench([&data, &counter]() {
                for (const std::string &s : data) {
                counter += amd64_simple_needs_escaping(s.c_str(), s.length() );
                }
            }));
  ...
  if(!amd64_branchless_needs_escaping(justquotes.c_str(), 64) || !amd64_branchless_needs_escaping(justbackslash.c_str(), 64)) {
    printf("AMD64 branchless implementation is not correct\n");
    return 1;
  }
  pretty_print(data.size(), volume, "AMD64 branchless_needs_escaping",
               bench([&data, &counter]() {
                 for (const std::string &s : data) {
                   counter += amd64_branchless_needs_escaping(s.c_str(), s.length());
                 }
               }));
